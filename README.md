...
- **专业版**：进一步解锁 **页面内重放 / Fuzz、HTTP/WS 微型 Fuzz、AI、AI 任务台、Skills 知识库、暗链、批量工作台与高级编解码** 等深度能力。
...
| **AI 任务 / AI 任务台（1.0.2 起）** | ❌ | ✅ | 多阶段自动化：智能渗透、CTF 夺旗；**Skills 知识库注入**（1.0.3 完善默认推荐与手动优先策略）、**运行中补充线索**；编排与工具链持续优化；联动历史、重放与验证并沉淀报告 |
| **AI Skills 知识库（1.0.3）** | ❌ | ✅ | 内置渗透/CTF 知识库与子模块；markdown 可编辑；**导入外部 SKILL.md / 技能目录**；高级设置与 AI 任务面板分层勾选；注入 AI 任务与单条/批量 AI 分析 |
...
> 发行包文件名以实际发布为准，文中示例为 `Hx0-HawkEye-Chrome-V1.0.3-Official.Release`（文件夹 / `.zip`）与 `Hx0-HawkEye-Firefox-V1.0.3.Official.Release`（文件夹 / `.xpi` / `.zip`）。版本号随发布更新。
>
...
3. 将下载好的 `Hx0-HawkEye-Chrome-V1.0.3-Official.Release.zip` 解压为 `.crx` 或直接拖拽已打包的 `.crx`（若发行方提供）**拖拽**到该扩展管理页面中。
...
1. 将下载的压缩包解压，得到 `Hx0-HawkEye-Chrome-V1.0.3-Official.Release` 文件夹，放到 **固定路径**（如 `D:\Tools\Hx0-Extension\`），**勿随意删除**。
...
4. 右上角 **齿轮** **⚙** → **「从文件安装附加组件…」**。
5. 选择 `Hx0-HawkEye-Firefox-V1.0.3.Official.Release.xpi` 并确认添加。
...
3. 进入解压后的 `Hx0-HawkEye-Firefox-V1.0.3.Official.Release` 文件夹，选中 `manifest.json` 打开。
4. 关闭浏览器后需 **重新执行本流程**。
