...
| **AI 分析（可选）** | **单包**：对 **请求 + 响应** 做解读、异常与风险辅助说明。**批量**：勾选多条历史，独立页 **逐条高亮 + 汇总报告**，适合 **多接口、多域名** 归纳与留证。**AI 任务台**：侧栏内多阶段自动化（智能渗透 / CTF），支持 **运行中补充线索** 注入后续轮次。模型支持 OpenAI、DeepSeek、本地 LM Studio、**自定义 Base URL**；兼容 **OpenAI 风格** 与 **千帆代码计划等** 完整路径；数据 **只发往您配置的 Endpoint**（BYOK）。 |
...
- **专业版**：进一步解锁 **页面内重放 / Fuzz、HTTP/WS 微型 Fuzz、AI、AI 任务台、暗链、批量工作台与高级编解码** 等深度能力。
...
| **AI 任务 / AI 任务台（1.0.2）** | ❌ | ✅ | 多阶段自动化：智能渗透、CTF 夺旗；**运行中补充线索**；编排与工具链持续优化；联动历史、重放与验证并沉淀报告 |
...
7. **AI 能力成环**：**AI 任务台**（多阶段 + **运行中补充线索**）、**AI 生成测试用例**、**AI 生成 Fuzz Payload**、**单包请求/响应 AI 分析**、**批量 AI 分析**（多包汇总，便于 **供应链 / 依赖面** 类线索归纳）与 **暗链规则 + AI 解读** 可在 **同一侧栏工作流** 内完成，少切工具。  
...
> 发行包文件名以实际发布为准，文中示例为 `Hx0-HawkEye-Chrome-V1.0.2-Official.Release`（文件夹 / `.crx`）与 `Hx0-HawkEye-Firefox-V1.0.2.Official.Release`（文件夹 / `.xpi`）。版本号随发布更新。
>
...
3. 将下载好的 `Hx0-HawkEye-Chrome-V1.0.2-Official.Release.crx` **拖拽**到该扩展管理页面中。
...
1. 将下载的压缩包解压，得到 `Hx0-HawkEye-Chrome-V1.0.2-Official.Release` 文件夹，放到 **固定路径**（如 `D:\Tools\Hx0-Extension\`），**勿随意删除**。
...
4. 右上角 **齿轮** **⚙** → **「从文件安装附加组件…」**。
5. 选择 `Hx0-HawkEye-Firefox-V1.0.2.Official.Release.xpi` 并确认添加。
...
3. 进入解压后的 `Hx0-HawkEye-Firefox-V1.0.2.Official.Release` 文件夹，选中 `manifest.json` 打开。
4. 关闭浏览器后需 **重新执行本流程**。
...
AI 任务模块会记录执行过程、请求证据、关键 payload、Flag 命中结果和最终分析报告；**运行中**可通过工具栏 **「补充线索」** 队列化注入后续轮次，**不替代**启动前任务背景；底层 **多阶段自适应编排** 持续加固（工具链、预算与可读性等）。详见用户手册 §8。
