...
| **重放工作台** | 编辑原始报文后一键重放；**页面内重放**（应对部分 WAF 动态页）；撤销/重做；目标域名切换；**AI 生成测试用例**：按当前请求与选项让模型输出结构化用例列表，快速铺测试思路（需配置 AI）。 |
| **编解码与哈希** | MD5、SM3、SHA、ROT13、Base64、URL、Hex 等；作用范围可选 **选中文本 / 仅参数值 / 整行 URL**。 |
| **微型 Fuzz** | 使用 `§...§` 标记注入点；**开始 Fuzz** 与 **页面内 Fuzz**；基线对比；**AI 智能 Payload**：由模型按注入上下文生成候选 payload 列表，减少手写字典成本；可结合 Render、敏感、**AI 结果分析**视图辅助判断。 |
...
- **社区版**：覆盖日常高频的 **抓包 → 看详情 → 普通重放 → 基础编解码** 主链路。
- **专业版**：进一步解锁 **主动控制流量、页面内重放 / Fuzz、AI、暗链、批量工作台与高级编解码**。
...
| 普通重放 | ✅ | ✅ | 社区版保留完整基础验证闭环 |
| **拦截开关、改包 / 放行 / 一键放行 / 一键丢弃（1.0.2 下放）** | ✅  | ✅ | 代表“主动控制流量”的专业能力 |
...
| 页面内重放、页面内 Fuzz、微型 Fuzz、标记注入点 | ❌ | ✅ | 适合动态页、WAF 与高频参数试探 |
| 切换请求方法、目标域名切换 | ❌ | ✅ | 适合多环境联调与验证 |
| **智能代理分流器（1.0.1 新增）** | ❌ | ✅ | 位于基础设置页“抓包类型 / 后缀”下方，可按站点规则将命中请求转发到 Burp、Yakit 或其他上游代理，未命中请求保持原网络路径；Firefox 版额外支持“兼容模式 / 接管模式” |
| AI 分析设置、AI 结果分析、AI 分析、AI 生成用例 | ❌ | ✅ | AI 能力统一归于专业版 |
| **AI 任务（1.0.2 新增）** | ❌ | ✅ | 让 AI 基于当前目标自动梳理攻击面、生成测试计划、联动重放/Fuzz 验证并沉淀证据，支持智能渗透和CTF夺旗两种模式。 |
| 暗链与静态威胁检测、报告下载、高信誉顶级域 | ❌ | ✅ | 规则扫描、报告与降噪能力 |
| 全量深度搜索、自定义正则 / 关键词库 | ❌ | ✅ | 深搜与规则扩展能力 |
| 批量导出、批量删除、批量重放、批量 AI 分析、批量暗链检测 | ❌ | ✅ | 批量工作台统一归于专业版 |

> `1.0.1` 版本主要新增两项专业版能力：`智能代理分流器` 与 `加密逻辑智能分析`。
> `1.0.2` 版本主要新增一项专业版能力：`AI 任务` ，下放到社区版一项专业版能力：`拦截开关、改包 / 放行 / 一键放行 / 一键丢弃`

### 一句话理解版本边界

- **社区版负责**：看包、理解、初步验证、基础编解码、控制流量。
- **专业版负责**：页面上下文验证、变异测试、AI 分析、AI任务、暗链扫描、批量产出。
...
| **重放 / Fuzz** | 重放台 + **微型 Fuzz** + **页面内重放/Fuzz** | Repeater / Intruder 成熟 | Web Fuzzer 等模块 | 通常无并发 Fuzz、无结构化对比 |
...
> 发行包文件名以实际发布为准，文中示例为 `Hx0-HawkEye-Chrome-V1.0.0-Official Release`（文件夹 / `.crx`）与 `Hx0-HawkEye-Firefox-V1.0.0-Official Release`（文件夹 / `.xpi`）。版本号随发布更新。
>
...
3. 将下载好的 `Hx0-HawkEye-Chrome-V1.0.0-Official Release.crx` **拖拽**到该扩展管理页面中。
...
1. 将下载的压缩包解压，得到 `Hx0-HawkEye-Chrome-V1.0.0-Official Release` 文件夹，放到 **固定路径**（如 `D:\Tools\Hx0-Extension\`），**勿随意删除**。
...
4. 右上角 **齿轮** **⚙** → **「从文件安装附加组件…」**。
5. 选择 `Hx0-HawkEye-Firefox-V1.0.0-Official Release.xpi` 并确认添加。
...
3. 进入解压后的 `Hx0-HawkEye-Firefox-V1.0.0-Official Release` 文件夹，选中 `manifest.json` 打开。
4. 关闭浏览器后需 **重新执行本流程**。
...
本次 `V1.0.2` 版本围绕重放能力、AI 自动化测试流程和社区版功能开放进行了重点升级，进一步提升 Web 安全测试、CTF 练习和日常抓包分析的效率。
...
AI 任务模块会记录执行过程、请求证据、关键 payload、Flag 命中结果和最终分析报告，方便后续复现与整理 Writeup。
