...
| **油猴脚本 / 页面脚本工作台（1.0.4）** | ❌ | ✅ | 导入 `.user.js`、脚本库管理、一键注入；AI 创建/优化脚本；与抓包重放等工具联动 |
...
3. 将下载好的 `Hx0-HawkEye-Chrome-V1.0.4-Official.Release.zip` 解压为 `.crx` 或直接拖拽已打包的 `.crx`（若发行方提供）**拖拽**到该扩展管理页面中。
